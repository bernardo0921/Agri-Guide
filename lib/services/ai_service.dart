...
class AIService {
  ...
  /// Gets a daily AI tip with caching
  /// The tip is cached until the next day
  static Future<Map<String, dynamic>> getDailyAITip() async {
    final token = await _getAuthToken();
    if (token == null) {
      return {'success': false, 'error': 'Not authenticated'};
    }
    ...
    try {
      ...
      final lastTipDate = prefs.getString('ai_tip_date');
      final today = DateTime.now();
      final todayString = '${today.year}-${today.month}-${today.day}';

      if (lastTipDate == todayString) {
        // Return cached tip if it's still today
        final cachedTip = prefs.getString('ai_tip_content');
        if (cachedTip != null) {
          return {'success': true, 'tip': cachedTip, 'cached': true};
        }
      }

      // Fetch new tip from server
      final response = await http.post(
        Uri.parse('$_baseUrl/ai-tip/'),
        headers: {
          'Authorization': 'Token $token',
          'Content-Type': 'application/json',
        },
        body: json.encode({'context': 'farming tips'}),
      );

      if (response.statusCode == 200) {
        final data = jsonDecode(response.body);
        final tipContent = data['tip'] ?? 'No tip available';

        // Cache the tip for today
        await prefs.setString('ai_tip_content', tipContent);
        await prefs.setString('ai_tip_date', todayString);

        return {'success': true, 'tip': tipContent, 'cached': false};
      } else if (response.statusCode == 401) {
        return {
          'success': false,
          'error': 'Authentication failed. Please login again.',
        ...
        return {'success': false, 'error': 'Failed to fetch AI tip'};
      }
    } catch (e) {
      return {'success': false, 'error': 'Request failed: $e'};
    }
